...
namespace Web::Routes::Pages {
	...
	inline void registerSettings(crow::SimpleApp& app) {
		CROW_ROUTE(app, "/settings")([]() {
			...
			std::vector<const Core::Config::dataEntry*> sorted;
			for (const Core::Config::dataEntry& e : Core::Config::data::arr)
				sorted.push_back(&e);
			std::sort(sorted.begin(), sorted.end(),
				[](const Core::Config::dataEntry* a, const Core::Config::dataEntry* b) { return a->sort < b->sort; });

			switch (Core::Config::application.server) {
				case Core::Config::server::bancho:  ctx["tracker_config_name"] = "Bancho Tracker Config";  break;
				case Core::Config::server::titanic: ctx["tracker_config_name"] = "Titanic Tracker Config"; break;
			}

			for (const Core::Config::dataEntry* e : sorted) {
				crow::json::wvalue el;
				el["id"]    = e->key;
				el["label"] = e->name;
				el["sort"]  = e->sort;
