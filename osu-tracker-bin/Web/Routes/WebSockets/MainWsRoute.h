...
namespace Web::Routes::WebSockets {
	...
	inline void registerMain(crow::SimpleApp& app) {
		...
		.onmessage([](crow::websocket::connection& conn, const std::string& data, bool is_binary) {
			...
			Core::Console::writeLog("Command received: " + cmd);
			...
			if (cmd[0] == '#') {
				...
				if (cmd == "#saveSettings") {
					...
					for (const std::string& k : { "banchoId", "titanicId", "clientId", "clientSecret", "gameMode", "server" }) {
						if (incoming.count(k) && incoming.at(k) != Core::Config::application.get(k)) {
							resetSession = true;
							break;
						}
					}
					...
					for (const auto& [key, val] : incoming)
						Core::Config::application.set(key, val);
					{
						std::unique_lock<std::shared_mutex> dataLock(Core::Config::data::dataMutex);
						for (const auto& item : j["msg"]["trackerConfig"]) {
							int idx = Core::Config::data::getIndex(item["key"].get<std::string>());
							if (idx < 0) continue;
							if (item.contains("display")) Core::Config::data::arr[idx].display = item["display"].get<bool>();
							if (item.contains("sort"))    Core::Config::data::arr[idx].sort    = item["sort"].get<int>();
						}
						// Immediately sync arrFormatted so the ImGui UI reflects
						// toggle/sort changes without waiting for the next API poll.
						Core::Formatter::updateFormat();
					}
					Core::Config::write();
					if (resetSession) {
