...
// Forward-declarations for symbols defined in ui.h (which includes this file).
...
// -- Theme colours (matching the web palette) ---------------------------------
...
int ui_main()
{
    ...
    // Extend beyond Basic Latin to include Geometric Shapes (U+2500–U+25FF) so
    // characters like ● (U+25CF) render correctly.
    ...
    ImFont* fontDefault = io.Fonts->AddFontFromMemoryTTF((void*)DroidSans_ttf, sizeof(DroidSans_ttf), 18.0f, &font_cfg, glyphRanges);
    ImFont* fontHeader  = io.Fonts->AddFontFromMemoryTTF((void*)DroidSans_ttf, sizeof(DroidSans_ttf), 22.0f, &font_cfg, glyphRanges);
    ImFont* fontSmall   = io.Fonts->AddFontFromMemoryTTF((void*)DroidSans_ttf, sizeof(DroidSans_ttf), 15.0f, &font_cfg, glyphRanges);
    ...
    if (!config::user.avatarBytes.empty())
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (ImGui::Begin(
            config::user.username.empty() ? "osu! Tracker" : config::user.username.c_str(),
            nullptr, beginFlags))
        {
            // -- Profile header ---------------------------------------------
            ...
            ImGui::TextUnformatted(config::user.username.empty() ? "No User" : config::user.username.c_str());
            ImGui::PopFont();

            // Server badge
            ImGui::PushFont(fontSmall);
            const char* serverLabel = (config::application.server == config::server::titanic) ? "Titanic" : "Bancho";
            const ImVec4 badgeCol   = (config::application.server == config::server::titanic)
            ...
            // -- Toolbar ---------------------------------------------------
            {
                ...
                for (int m = 0; m < 4; m++) {
                    bool active = (int)config::application.gameMode == m;
                    btnStyle(active);
                    if (ImGui::Button(modeLabels[m], ImVec2(modeW, 0)) && !active) {
                        config::application.gameMode = (config::gameMode)m;
                        config::write();
                        ui_restartFetch();
                    }
                ...
                for (int sv = 0; sv < 2; sv++) {
                    bool active = (int)config::application.server == sv;
                    btnStyle(active);
                    if (ImGui::Button(srvLabels[sv], ImVec2(serverW, 0)) && !active) {
                        config::application.server = (config::server)sv;
                        config::write();
                        ui_restartFetch();
                    }
                    ImGui::PopStyleColor(4);
                ...
                // Settings button – right-aligned on the server row
                ImGui::SameLine();
                ImGui::SetCursorPosX(
...

            {
                ...
                for (auto& entry : config::data::arrFormatted)
                {
                    ...
                    switch (config::application.server) {
                        case config::server::bancho:  if (!entry.banchoSupport)  continue; break;
                        case config::server::titanic: if (!entry.titanicSupport) continue; break;
                    }
        ...
        if (debug) {
            ...
            if (ImGui::Begin("##debug", nullptr, debugFlags))
            {
                ...
                if (ImGui::CollapsingHeader("  THREADS", ImGuiTreeNodeFlags_DefaultOpen))
                {
                    ...
                    if (ui::fetchCount.load() > 0) {
                        char tmp[128];
                        snprintf(tmp, sizeof(tmp), "last %s  (%lldms)",
                            fmtTime(ui::lastFetchCompleted).c_str(),
                            (long long)ui::lastFetchDuration.count());
                        dbgRow("", tmp);

                        auto nextAt   = ui::lastFetchCompleted + std::chrono::milliseconds(config::application.apiInterval);
                        ...
                        snprintf(tmp, sizeof(tmp), "cycle #%d  |  next in %lldms",
                            ui::fetchCount.load(), remMs > 0 ? (long long)remMs : 0LL);
                        dbgRow("", tmp);
                    } else {
                ...
                {
                    ImGui::Spacing();
                    dbgRow("Server",     config::application.server == config::server::titanic ? "Titanic" : "Bancho");
                    dbgRow("Init fail",  api::init_api_failed ? "YES" : "no");
                    dbgRow("Avatar URL", config::user.avatarUrl.empty() ? "-" : config::user.avatarUrl.c_str());
                    ...
                    auto apiRow = [&](const char* name, const api::EndpointStatus& st) {
                        ImGui::PushStyleColor(ImGuiCol_Text, COL_MUTED);
                    ...
                    if (config::application.server == config::server::bancho) {
                        apiRow("Auth",       api::statusAuth);
                        apiRow("Bancho",     api::statusBancho);
                        apiRow("Respektive", api::statusRespektive);
                        apiRow("Inspector",  api::statusInspector);
                    } else {
                        apiRow("Titanic",    api::statusTitanic);
                    }
                    ImGui::Spacing();
                }
                ...
                {
                    ImGui::Spacing();
                    dbgRow("Username", config::user.username.empty() ? "-" : config::user.username.c_str());

                    std::shared_lock<std::shared_mutex> dataLock(config::data::dataMutex);
                    char tmp[16]; snprintf(tmp, sizeof(tmp), "%d", (int)config::data::arr.size());
                    dbgRow("Entries", tmp);
                    ImGui::Spacing();

                    const int fmtSize = (int)config::data::arrFormatted.size();
                    for (size_t j = 0; j < config::data::arr.size(); j++) {
                        const auto& raw = config::data::arr[j];
                        if (ImGui::CollapsingHeader(raw.key.c_str())) {
                            if ((int)j >= fmtSize) continue;
                            const auto& fmt = config::data::arrFormatted[j];
