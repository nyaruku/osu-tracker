...
namespace config {
	...
	struct AppConfig {
		...
		int activeId() const {
			return (server == config::server::titanic) ? titanicId : banchoId;
		}

		void set(std::string_view key, const std::string& value) {
			switch (hash(key)) {
				case hash("osuId"):        banchoId     = std::stoi(value);                              return; // backward compat
	...
	// Reset everything to compiled-in defaults (used by Reset Settings).
			...
			if (std::ifstream file{ entry.path() }; file.is_open()) {
				for (std::string line; std::getline(file, line); ) {
					for (size_t i = 0; i < data::arr.size(); ++i) {
						...
						line = ext::replace(line, "{{" + k + "_init_raw}}",    raw.init);
						line = ext::replace(line, "{{" + k + "_change_raw}}", raw.change);
						line = ext::replace(line, "{{" + k + "_current_raw}}", raw.current);
						line = ext::replace(line, "{{" + k + "_init}}",        fmt.init);
						line = ext::replace(line, "{{" + k + "_change}}",      fmt.change);
						line = ext::replace(line, "{{" + k + "_current}}",     fmt.current);
					}
					content += line + '\n';
				}
			} else {
	...
	inline void readEnv(const std::string& path = "debug.env") {
		if (std::ifstream file{ path }; file.is_open()) {
			...
			for (std::string line; std::getline(file, line); ) {
				...
				auto [key, value] = ext::split2tuple(line, '=');
