...
namespace Web::TemplateRegister {
	...
	inline void scan(const std::string& dir = "./www/template/") {
		...
		if (!std::filesystem::exists(dir)) {
			console::writeLog("TemplateRegistry: directory not found: " + dir, true, 255, 165, 0);
			return;
		}
		...
		for (const auto& entry : std::filesystem::directory_iterator(dir)) {
			...
			console::writeLog("TemplateRegistry: registered template \"" + title + "\" -> /template/" + stem, false);
		}
