...
void drawContent(struct nk_context* ctx, struct nk_font* font, struct nk_font* fontSmall, struct nk_font* fontHeader, int w, int h, struct appC app, struct userC user, struct dataEntryC* entries, size_t count, bool debug, bool data_debug) {
	...
	if (nk_begin(ctx, custom_strcat_static(3, "[color=#FFFFFF]", _username, "[/color]"), nk_rect(0, 0, w, h),NK_WINDOW_TITLE| NK_WINDOW_BACKGROUND))
	{	
		int nkEditHeight = 0;
		#ifdef _WIN32
		...
		for (size_t i = 0; i < count; i++) {
			...
			if (entries[i].name == NULL) {
				...
				_name = entries[i].name;
			}

			nk_label(ctx, _name, NK_TEXT_ALIGN_LEFT);

			const char* _current;
			if (entries[i].current == NULL) {
				_current = "";
			} else {
				_current = entries[i].current;
			}
			...
			if (!entries[i].single) {
				...
				const char* _change;
				if (entries[i].change == NULL) {
					_change = "";
				} else {
					_change = entries[i].change;
				}
